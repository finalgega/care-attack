...
public class User {
	...
	public User(String username, String password, String email, String secret,
			String secretAnswer, String profilePic) {
		super();
		this.username = username;
		this.password = password;
		this.email = email;
		this.secret = secret;
		this.secretAnswer = secretAnswer;
		this.profileImgPath = profilePic;
	}
	...
	public String getImageData()
	{
		...
		MySQLController mysql = new MySQLController();
		mysql.setUp();
		String dbQuery = "SELECT imagePath FROM accounts ac INNER JOIN image i ON ac.imageID = i.imageID";
		try{
			ResultSet r;
			r = mysql.readRequest(dbQuery);
			rs = r.getString("imagePath");
